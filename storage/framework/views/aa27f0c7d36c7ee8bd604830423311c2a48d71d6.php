					...
					<div class="x_content">
						...
						<?php if(getActiveCustomer(Auth::user()->id)=='yes' || getActiveEmployee(Auth::user()->id)=='yes'): ?>
							<?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sales_view')): ?>
								<li role="presentation" class="active"><a href="<?php echo url('/sales/list'); ?>"><span class="visible-xs"></span> </span><i class="fa fa-list fa-lg">&nbsp;</i><b><?php echo e(trans('app.List Of Vehicle Sale')); ?></b></a></li>
							<?php endif; ?>
							<?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sales_add')): ?>
								<li role="presentation" class=""><a href="<?php echo url('/sales/add'); ?>"><span class="visible-xs"></span> <i class="fa fa-plus-circle fa-lg">&nbsp;</i><?php echo e(trans('app.Add Vehicle Sale')); ?></span></a></li>
							<?php endif; ?>
						...
						<table id="datatable" class="table table-striped jambo_table" style="margin-top:20px;">
							<thead>
								<tr>
									...
									<th><?php echo e(trans('app.Model Name')); ?></th>
							...
							<tbody>
									...
									<tr>
										...
										<td><?php echo e(getModelName($sale->vehicle_id)); ?></td>
										<td><?php echo e(getAssignedName($sale->salesmanname)); ?></td>
										<td><?php echo e(getAssignedName($sale->assigne_to)); ?></td>
										...
										<?php if(getUserRoleFromUserTable(Auth::User()->id) == 'admin' || getUserRoleFromUserTable(Auth::User()->id) == 'supportstaff' || getUserRoleFromUserTable(Auth::User()->id) == 'accountant' || getUserRoleFromUserTable(Auth::User()->id) == 'employee'): ?>
											...
											<?php if($sales_invoice == "No data"): ?>
												<?php if(Gate::allows('sales_add')): ?>
													<?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sales_add')): ?>
														<a href="<?php echo url('invoice/add/'.$sale->id); ?>" ><button type="button" class="btn btn-round btn-info"><?php echo e(trans('app.Create Invoice')); ?></button></a>
													<?php endif; ?>
												<?php else: ?>
													<?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sales_view')): ?>
											...
											<?php else: ?>
												...
												<?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sales_view')): ?>
												<button type="button" data-toggle="modal" data-target="#myModal" saleid="<?php echo e($sale->id); ?>" invoice_number="<?php echo e(getInvoiceNumber($sale->id)); ?>" url="<?php echo url('/sales/list/modal'); ?>" class="btn btn-round btn-info save"><?php echo e(trans('app.View Invoices')); ?></button>
												<?php endif; ?>
											<?php endif; ?>
												
											<?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sales_edit')): ?>
												<a href="<?php echo url('sales/list/edit/'.$sale->id); ?>" ><button type="button" class="btn btn-round btn-success"><?php echo e(trans('app.Edit')); ?></button></a>
											<?php endif; ?>
											
											<?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sales_delete')): ?>
												<a url="<?php echo url('sales/list/delete/'.$sale->id); ?>" class="sa-warning"><button type="button" class="btn btn-round btn-danger"><?php echo e(trans('app.Delete')); ?></button></a>
											<?php endif; ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\garage\resources\views/sales/list.blade.php ENDPATH**/ ?>